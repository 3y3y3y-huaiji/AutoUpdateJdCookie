...
class CaptchaSolver:
    ...
    async def solve_slider_captcha(
        ...
        for i in range(retry_times):
            ...
            try:
                ...
                background_img_path = self._save_img('background_img', background_img_bytes)

                small_img_width = await page.evaluate('() => { return document.getElementById("slot_img").clientWidth; }')
                small_img_height = await page.evaluate('() => { return document.getElementById("slot_img").clientHeight; }')

                small_image = Image.open(small_img_path)
                resized_small_image = small_image.resize((small_img_width, small_img_height))
                resized_small_image.save(small_img_path)

                background_img_width = await page.evaluate('() => { return document.getElementById("main_img").clientWidth; }')
                background_img_height = await page.evaluate('() => { return document.getElementById("main_img").clientHeight; }')

                background_image = Image.open(background_img_path)
                resized_background_image = background_image.resize((background_img_width, background_img_height))
                ...
                if move_solve_type == "old":
                    distance = self._ddddocr_find_bytes_pic(small_img_bytes, background_img_bytes)
                    await asyncio.sleep(1)
                    await self._solve_slider_captcha(page, slider, distance, slide_difference)
                    await asyncio.sleep(1)
                else:
                    distance = self._ddddocr_find_files_pic(small_img_path, background_img_path)
                    await asyncio.sleep(1)
                    await self._new_solve_slider_captcha(page, slider, distance, slide_difference)
                    await asyncio.sleep(1)
    ...
    async def solve_shape_captcha(self, page: Page, retry_times: int = 5) -> bool:
        ...
        for i in range(retry_times + 1):
            ...
            try:
                ...
                if await page.locator("div.sp_msg.tip_text", has_text="请点击上图中的").is_visible():
                    logger.info("检测为图像，开始图像识别")
                    success = await self._solve_image_captcha(
                        page, rgba_word_img_path, background_img_path,
                        backend_bounding_box, refresh_button
                    )
                    if success:
                ...
                if '色' in word:
                    success = await self._solve_color_captcha(
                        page, word, background_img_path,
                        backend_bounding_box, button, refresh_button
                    )
                    if success:
                        continue
    ...
    def _ddddocr_find_bytes_pic(self, target_bytes: bytes, background_bytes: bytes) -> int:
        det = ddddocr.DdddOcr(det=False, ocr=False, show_ad=False)
        res = det.slide_match(target_bytes, background_bytes, simple_target=True)
        return res['target'][0]

    async def _solve_slider_captcha(self, page: Page, slider, distance: int, slide_difference: int):
    ...
    async def _new_solve_slider_captcha(self, page: Page, slider, distance: int, slide_difference: int):
        distance = distance + slide_difference
        box = await slider.bounding_box()
        await page.mouse.move(box['x'] + 10, box['y'] + 10)
        await page.mouse.down()
        await page.mouse.move(box['x'] + distance + random.uniform(8, 10), box['y'], steps=5)
        await asyncio.sleep(random.randint(1, 5) / 10)
        await page.mouse.move(box['x'] + distance, box['y'], steps=10)
        await page.mouse.up()
        await asyncio.sleep(3)

    async def _human_like_mouse_move(self, page: Page, from_x: float, to_x: float, y: float):
        fast_duration = 0.28
        fast_steps = 50
    ...
    def _rgba2rgb(self, img_name: str, rgba_img_path: str, tmp_dir: str = './tmp') -> str:
        import os
        from PIL import Image
        os.makedirs(tmp_dir, exist_ok=True)
    ...
    async def _solve_image_captcha(
        self, page: Page, rgba_word_img_path: str,
        background_img_path: str, backend_bounding_box: dict,
        refresh_button
    ) -> bool:
        try:
            ...
            target_dict = self._ddddocr_find_files_pic_v2(small_img_path, background_img_path)
            ...
            x, y = backend_bounding_box['x'] + center_x, backend_bounding_box['y'] + center_y
    ...
    def _ddddocr_find_files_pic_v2(self, target_file: str, background_file: str) -> dict:
        with open(target_file, 'rb') as f:
            target_bytes = f.read()
        with open(background_file, 'rb') as f:
            background_bytes = f.read()
    ...
    async def _solve_color_captcha(
        self, page: Page, word: str, background_img_path: str,
        backend_bounding_box: dict, button, refresh_button
    ) -> bool:
    ...
    async def _solve_text_captcha(
        ...
        import re
        from utils.tools import get_shape_location_by_type, expand_coordinates, cv2_save_img
