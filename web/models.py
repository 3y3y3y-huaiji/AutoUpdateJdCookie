...
class AccountConfig(BaseModel):
    ...
    sms_func: Optional[Literal["no", "manual_input", "webhook"]] = Field(default=None, description="短信验证码处理方式")
    sms_webhook: Optional[str] = Field(default=None, description="短信验证码webhook地址")
    voice_func: Optional[Literal["no", "manual_input"]] = Field(default=None, description="语音验证码处理方式")
...
class NotificationConfig(BaseModel):
    ...
    send_webhook: List[str] = Field(default_factory=list, description="自定义Webhook地址")
    ...
    send_pushplus: List[str] = Field(default_factory=list, description="PushPlus通知地址")
    send_serverchan: List[str] = Field(default_factory=list, description="Server酱通知地址")
...
class AppConfig(BaseModel):
    ...
    包含所有应用配置的根模型
    """
    user_datas: Dict[str, AccountConfig] = Field(default_factory=dict, description="用户账号配置")
